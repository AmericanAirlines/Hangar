...
export const scoreVotes = (
  ...
  for (let i = 0; i < votes.length; i += 1) {
    if (!calibrationComplete) {
      ...
      const project = projects.sort((projectA, projectB) =>
        (calibrationCount[projectA.id] || 0) <= (calibrationCount[projectB.id] || 0) ? -1 : 1,
      )[0];
      if (project) {
        // Retrieve all remaining votes where the selected project is evaluated
        const projectVotes = remainingVotes.filter(
          (voteToCompare) =>
            voteToCompare.currentProject.id === project.id ||
            voteToCompare.previousProject.id === project.id,
        );

        // Sort project votes by the project with the lowest calibration count

        const vote = projectVotes.sort((a, b) => {
          const otherProjectIdA =
            a.currentProject.id !== project.id ? a.currentProject.id : a.previousProject.id;
          const otherProjectIdB =
            b.currentProject.id !== project.id ? b.currentProject.id : b.previousProject.id;

          return (calibrationCount[otherProjectIdA] || 0) <=
            (calibrationCount[otherProjectIdB] || 0)
            ? -1
            : 1;
        })[0];
        if (vote) {
          ...
          const otherProjectId =
            vote.currentProject.id !== project.id
              ? vote.currentProject.id
              : vote.previousProject.id;
          const otherProject = projects.find(
            (projectToCompare) => projectToCompare.id === otherProjectId,
          );
          if (otherProject) {
            ...
            const currentProjectScoreImpact = vote.currentProjectChosen
              ? calibrationScoreImpact
              : -calibrationScoreImpact;
            ...
            const allProjectsAreInCalibration =
              Object.keys(calibrationCount).length === projects.length;
            const allProjectsAreCalibrated = !Object.values(calibrationCount).some(
              (value) => value < votesNeededForCalibration,
            );
      ...
      if (vote) {
        ...
        if (currentProjectId && previousProjectId) {
          ...
          if (currentProjectScore && previousProjectScore) {
            ...
            const currentProjectPercentile =
              (currentProjectScore.score + normalizationShift) / normalizedMaxScore || 0;
            const previousProjectPercentile =
              (previousProjectScore.score + normalizationShift) / normalizedMaxScore || 0;
            ...
            // Apply score impact based on vote outcome
            currentProjectScore.score +=
              (vote.currentProjectChosen ? 100 : -100) + currentProjectScoreImpact;
            previousProjectScore.score +=
              (vote.currentProjectChosen ? -100 : 100) + previousProjectScoreImpact;
