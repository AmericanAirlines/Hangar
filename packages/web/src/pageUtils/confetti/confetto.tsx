...
export const Confetto:React.FC<ConfettoProps> = ({ tick=1000 , velocity:initialVelocity , right , zAxisClamp=1.3 }) => {
  // state management
  const
      [ shape ] = useState(randomShape) ,
      [ , advance ] = useState(0) ,
  ...
  // css
  const
    biezer = velocity.current.y>0 ? 'ease-out' : 'linear' ,
    frameDuration = velocity.current.y>0 ? tick : 250 ,
    { left , bottom , scale } =
      Object.entries( position.current )
